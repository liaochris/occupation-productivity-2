...
data_all_expanded <- data_all_expanded[subcell_code == "00" & owner_code == 1 &
                                         !is.na(as.numeric(data_all_expanded$value))]
data_all_expanded$value <- as.numeric(data_all_expanded$value)

data_all_expanded[,`estimate_datatype_code`:=paste(estimate_code, datatype_code, sep = "_")]

#mean of all quarters
meaned_vals <- data_all_expanded[, mean(value), 
                                 by = list(`occupation_code`,`occupation_text`, `year`,
                                           `industry_code`, `industry_text`, `area_code`, `area_text`,
                                           `estimate_datatype_code`)]

#generating wide tables for labor productivity data to make merging easier
EC_tbl1 <- dcast(meaned_vals, occupation_code + year + occupation_text + 
                   industry_code + industry_text + area_code + area_text ~  
                   estimate_datatype_code, value.var = c("V1"))
colnames(EC_tbl1) <- paste(colnames(EC_tbl1), "CM", sep = "_")

colnames(EC_tbl1) <- toupper(colnames(EC_tbl1))

EC_tbl1[`AREA_CODE_CM` == 99999, `AREA_CODE_CM`:=99]

cols <- c("estimate_datatype_code", 'estimate_code', 
          'estimate_text', "datatype_code", "datatype_text")
ec_desc <- unique(data_all_expanded[,..cols])
ec_desc$estimate_datatype_code <- paste(ec_desc$estimate_datatype_code, "CM", sep = "_")
